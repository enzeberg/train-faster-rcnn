    ...
    if len(train_loader) > 0:
        for i in range(2):
            ...
            pred_classes = [classes[i]
                            for i in targets[i]['labels'].cpu().numpy()]
...
def set_training_dir(folder_name=None):
    """
    This functions counts the number of training directories already present
    and creates a new one in `runs/train/`.
    And returns the directory path.
    """
    if not os.path.exists('runs/train'):
        os.makedirs('runs/train')
    if folder_name:
        new_dir_name = f"runs/train/{folder_name}"
        os.makedirs(new_dir_name, exist_ok=True)
        return new_dir_name
    else:
        num_train_dirs_present = len(os.listdir('runs/train/'))
        next_dir_num = num_train_dirs_present + 1
        new_dir_name = f"runs/train/res_{next_dir_num}"
        os.makedirs(new_dir_name, exist_ok=True)
        return new_dir_name